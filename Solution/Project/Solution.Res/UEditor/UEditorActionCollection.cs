...
namespace WeiFos.Res.UEditor
{
    public class UEditorActionCollection : Dictionary<string, Action<HttpContext>>
    {
        ...
        {
            if (ContainsKey(action)) this[action] = handler;
            else base.Add(action, handler);
        ...
        {
            new ListFileManager(context,Config.GetString("imageManagerListPath"),Config.GetStringList("imageManagerAllowFiles")).Process();
        }

        private void ListFileAction(HttpContext context)
        {
            new ListFileManager(context,Config.GetString("fileManagerListPath"), Config.GetStringList("fileManagerAllowFiles")).Process();
        }
